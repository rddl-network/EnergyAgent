    ...
    if array is not None:
        for structure in array.findall('Structure'):
            structure_data = {
                ...
                'octetString': structure.find('OctetString').get('Value') if structure.find('OctetString') is not None else None,
                'int8': structure.find('Int8').get('Value') if structure.find('Int8') is not None else None,
                'uint16_2': structure.findall('UInt16')[1].get('Value') if len(structure.findall('UInt16')) > 1 else None
            }
