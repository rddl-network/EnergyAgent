...
def test_getAccountInfo_Valid():
    accountID, sequence, statusText = getAccountInfo( "http://localhost:1317", "plmnt196ucf7y7t6x7kuvzlx5rpju5822efjelmn7xz0")
    assert 10 == accountID
    assert 0 == sequence
    assert "" == statusText

def test_getAccountInfo_Invalid():
    accountID, sequence, statusText = getAccountInfo( "http://localhost:1317", "plmnt196ucf707t6x7kuvzlx5rpju5822efjelmn7xz0")
    assert 0 == accountID
    assert 0 == sequence
    assert '{"code":2,"message":"decoding bech32 failed: invalid checksum (expected xwetnc got mn7xz0)","details":[]}' == statusText
...
def test_attestMachine_valid():
    ...
    assert "" == statusText
    tx = attestMachine( address, "MyMachine0", issuerPlanetmint, 
                  issuerLiquid, gps, deviceDefinition,
                  machine_id, signature, "", "planetmintgo",
                  accountID, sequence)
    print(tx)
