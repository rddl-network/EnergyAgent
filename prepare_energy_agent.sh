...
CRONJOB_UPGRADE="*/30 * * * *   cd ~/energy-agent && sleep \$(shuf -i 0-1500 -n 1) && docker system prune -f --all && docker compose up -d"
...
sudo /bin/bash -c "echo 'RuntimeMaxUse=300M' >> /etc/systemd/journald.conf"
